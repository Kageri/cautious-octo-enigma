...
with st.form("add_dog_form"):
    ...
    if st.form_submit_button("Add Dog"):
        if dog_name:  # ensure name is provided
...
for idx, dog in enumerate(st.session_state.dogs):
    # Ensure dog is a dict
    if isinstance(dog, dict):
        st.markdown(f"**{dog.get('name','Unnamed')}**")
        with st.expander("Edit Dog Info"):
            new_name = st.text_input("Name", value=dog.get("name",""), key=f"name_{idx}")
            new_breed = st.text_input("Breed", value=dog.get("breed",""), key=f"breed_{idx}")
            new_age = st.number_input("Age", min_value=0, max_value=25, value=dog.get("age",0), step=1, key=f"age_{idx}")
